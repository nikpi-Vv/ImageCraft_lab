...
static int is_positive_integer(double value) {
    if (value < 1.0) return 0;
    // Проверяем, что дробная часть близка к нулю
    return (fabs(value - round(value)) < 1e-5);
}
        ...
        if (expected_params > 0) {
            ...
            for (int j = 0; j < expected_params; j++) {
                ...
                if (strcmp(name, "crop") == 0 || strcmp(name, "med") == 0) {
                    ...
                    if (params[j] <= 0) {
                        printf("Error: '%s' requires positive sigma/radius, got %g\n", token, params[j]);
                        free(params);
                        free_filter_list(*out_head);
                        return 0;
                    }
                    ...
                    if (params[j] < 0 || params[j] > 1) {
                        printf("Error: '-edge' threshold must be in [0, 1], got %g\n", params[j]);
                        free(params);
                        free_filter_list(*out_head);
                        return 0;
                    }
                } else if (strcmp(name, "crystallize") == 0) {
                    if (!is_positive_integer(params[0])) {
                        printf("Error: '-crystallize' requires positive integer, got %g\n", params[0]);
                        free(params);
                        free_filter_list(*out_head);
                        return 0;
                    }
